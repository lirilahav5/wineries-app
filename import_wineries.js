...
async function importWineShops() {
  const geojson = JSON.parse(
    fs.readFileSync('wineries-app/src/assets/data/wine_shops.geojson', 'utf8')
  );
  ...
  for (const feature of geojson.features) {
    ...
    // Prepare the row for wine_shops table
    const row = {
      name: props.name,
    ...
    const { error } = await supabase.from('wine_shops').insert([row]);
    if (error) {
      console.error('Insert error:', error, 'Row:', row);
    } else {
      console.log('Inserted wine shop:', row.name);
    }
  }

  console.log('Finished importing wine_shops.');
}
