...
function WineShopsManagement() {
  ...
  const handleSave = async () => {
    try {
      ...
      alert('Error saving wine shop. Please try again.');
    }
  };
          ...
          <table style={{
            ...
            <tbody>
              {shops.map((shop) => (
                ...
                >
                  <td style={{ padding: '0.75rem', color: isDark ? '#ccc' : '#666' }}>{shop.id}</td>
                  <td style={{ padding: '0.75rem', fontWeight: '500' }}>{shop.name || '-'}</td>
                  <td style={{ padding: '0.75rem' }}>{shop.region || '-'}</td>
                  <td style={{ padding: '0.75rem', maxWidth: '200px', wordBreak: 'break-word' }}>{shop.address || '-'}</td>
                  <td style={{ padding: '0.75rem', whiteSpace: 'nowrap' }}>{shop.phone || '-'}</td>
                  ...
                  <td style={{ padding: '0.75rem', textAlign: 'center' }} key={`status-${shop.id}-${timeKey}`}>
                    {shop.opening_hours ? (() => {
                  ...
                  <td style={{ padding: '0.75rem', fontSize: '0.85rem', fontFamily: 'monospace' }}>
                    {shop.lat && shop.lng ? `${shop.lat.toFixed(4)}, ${shop.lng.toFixed(4)}` : '-'}
                  </td>
                  <td style={{ padding: '0.75rem', textAlign: 'center' }}>
                    {shop.logo_paid ? (
